...
/**
* @description 填充数据
* @param {location} 插入位置 string
* @param {json} 数据 json
* @param {displayAttrs} 显示的列 Array<string,string>
*/
...
function fillingDataWithTable(location, json, tname, displayAttrs){
	...
	//制作表头
	var attrs = typeof(displayAttrs)=="undefined"?jsonAttrName:displayAttrs;
...
// 翻页插件
function pageInit(json,displayFun){
	var index                      = json["index"];
	var pages                      = json["pages"];
	var items                      = json["total"];
	var itemsOnPage                = json["pagesize"];
	var pagePlugin				   = $("[data-uk-pagination]");
	var pagination                 = $.UIkit.pagination(".uk-pagination");
	pagination.options.currentPage = index;
	pagination.options.pages       = pages;
	var i = 1;
	pagination.options.onSelectPage= function(e){
		console.log(i++);
	};
	pagination.init();
}
